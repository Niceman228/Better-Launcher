...
| Метрика | Оригинал | BИLauncher 5.4 |
|---|---|---|
...
- рабочий стол всегда восстанавливается из Room и не теряет ярлыки после блокировки экрана;
- обновление приложения безопасно выходит из скрытого режима, а локальный обработчик клавиш страхует отключившийся AccessibilityService;
- исправлено задвоение селектора при D-pad навигации.

### Новые функции

- Кнопки вызова и контактов на главном экране в кнопочном режиме;
- отключение Wi-Fi и Bluetooth в скрытом режиме; мобильный интернет остаётся включён, а системный слот уровня сигнала и 3G/4G/LTE скрывается;
...
Для скрытия мобильного индикатора без root и Shizuku один раз выдайте
development-разрешение при подключённом ADB:

```bash
adb shell pm grant com.customlauncher.app android.permission.WRITE_SECURE_SETTINGS
```

Разрешение сохраняется после перезагрузки и обновлений приложения.
